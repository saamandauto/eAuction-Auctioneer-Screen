...
const createWatcherInfo = (count: number, dealers: any[]): ViewerInfo[] => {
...
export const MOCK_WATCHERS = new Map<number, ViewerInfo[]>();

let cachedDealers: any[] = [];

export const updateMockWatchers = (lots: LotDetails[], dealers?: any[]) => {
  if (dealers) {
    cachedDealers = dealers;
  }
  
  lots.forEach(lot => {
    MOCK_WATCHERS.set(lot.lotNumber, createWatcherInfo(lot.watchers, cachedDealers));
  });
};