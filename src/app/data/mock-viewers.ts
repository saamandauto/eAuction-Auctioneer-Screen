...
const createViewerInfo = (count: number, dealers: any[]): ViewerInfo[] => {
...
export const MOCK_VIEWERS = new Map<number, ViewerInfo[]>();

let cachedDealers: any[] = [];

export const updateMockViewers = (lots: LotDetails[], dealers?: any[]) => {
  if (dealers) {
    cachedDealers = dealers;
  }
  
  lots.forEach(lot => {
    MOCK_VIEWERS.set(lot.lotNumber, createViewerInfo(lot.viewers, cachedDealers));
  });
};