...
const createLeadInfo = (count: number, dealers: any[]): ViewerInfo[] => {
...
export const MOCK_LEADS = new Map<number, ViewerInfo[]>();

let cachedDealers: any[] = [];

export const updateMockLeads = (lots: LotDetails[], dealers?: any[]) => {
  if (dealers) {
    cachedDealers = dealers;
  }
  
  lots.forEach(lot => {
    MOCK_LEADS.set(lot.lotNumber, createLeadInfo(lot.leadListUsers, cachedDealers));
  });
};